...
#define SORT_ONEGIN "---------------------SORT ONEGIN---------------------\n"
#define SORT_ONEGIN_FROM_END "---------------------SORT ONEGIN FROM END---------------------\n"
#define ORIGINAL_ONEGIN "---------------------ORIGINAL ONEGIN---------------------\n"

size_t get_file_size(const char* file_name, struct stat* buf) {
    assert(file_name);
    assert(buf);
...
int main() {
    ...
    size_t file_size = get_file_size("test_file.txt", &buf);
    //printf("%d\n", file_size);
    ...
    bubble_sort(string_ptrs, num_of_strings);
